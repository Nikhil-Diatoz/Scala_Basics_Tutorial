...
//reading input from user examples
var num = scala.io.StdIn.readLine("What's your age? ").toInt //reading line then converting to int
var num1 = scala.io.StdIn.readInt() //reading line with int

def printPrime(): Unit ={
  /*
   * simple function for getting prime numbers.
   * in scala we don't have break condition and continue we can use below technique to achieve the same
   */
  val prime_list = List(1,2,3,4,5,11)
  for(i <- prime_list){
    if (i==11){
      return
    }
...
def input_output(): Unit ={
  /*
   * Function that takes an input from user and prints the number if that matches the required number
   * to input we can give "readInt, readDouble, readByte, readShort, readLong" options to load specific data type
   * alternatively other method readLine.type (choose_data_type Like toInt, toChar..etc)
   * this is a simple application to get the guessed random number specified in program
   */
  var num_guess = 0
  do{
    println("Guess a number")
    num_guess = readLine.toInt
  }while(num_guess != 15)
